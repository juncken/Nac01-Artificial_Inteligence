...
def calculo(mask, frame):
    ...
    if len(centers) == 2:
        ...
        if (angulo_graus > 0 and angulo_graus < 90):
            print('Tecla: ', keys[3])
            time.sleep(0.15)
        ...
        elif (angulo_graus < 0 and angulo_graus > -90):
            print('Tecla: ', keys[2])
            time.sleep(0.15)
...
while rval:
    ...
    circles = cv2.HoughCircles(cframe, cv2.HOUGH_GRADIENT, dp=5, minDist=200, param1=300, param2=200, minRadius=50,
                               maxRadius=70)
